...
object WikipediaRanking {
   ...
   */
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.aggregate(0)((count, article)=> article.mentionsLanguage(lang)match {case true=> count + 1 case_ => count + 0}, (a+b)=> a+b)
   ...
   */
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = langs.map(lang => (lang, occurencesOfLang(lang, rdd))).sortBy(_._2).reverse
   ...
   */
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = rdd.flatMap(a=> langs.map(1 => a.mentionsLanguage(1)match{ case true => (1,a)case flase => null})).filter(_ !=null).groupByKey()
   ...
   */
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] =  index.mapValues(u=>u.size).filter(_._2 != 0).sortBy(_._2).collect().toList
